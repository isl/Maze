 ...
 */
public class TreeMappingRules_Generator {
    
    private static Logger logger = Logger.getLogger(TreeMappingRules_Generator.class);
    private X3ML x3ml;
    private MappingRulesSchema mappingRulesSchema;
    private TreeSourceSchema_MappingRules treeMappingRules;
    private Document sourceSchemaDoc;
    
    
    public TreeSourceSchema_MappingRules createTreeMappingRules(X3ML x3mlObj, Document sourceDoc){
        try {
            logger.info("Start generating mapping rules....");
            this.x3ml = x3mlObj;
            this.sourceSchemaDoc = sourceDoc;
            this.mappingRulesSchema = new MappingRulesSchema();
            this.treeMappingRules = new TreeSourceSchema_MappingRules();
            
            Node root = this.sourceSchemaDoc.getDocumentElement();
            ...
            this.treeMappingRules.setMappingRulesSchema(mappingRulesSchema);
            logger.info("Mapping Rules Finished!");
            try{
                logger.info("[Mapping Rules Count]: " + mappingRulesSchema.getClasses().size());
            }catch(Exception e){}
            
            return this.treeMappingRules;
        } catch (Exception e) {
            logger.error("Cannot create TREE Mapping rules for:"+x3ml.getInfo().getTitle(), e);
            return null;
        }
    }
